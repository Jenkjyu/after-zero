...
- 根 `ios/` 已是可编译、可在 iPhone 运行的 Capacitor 8.4.1 Swift Package Manager 原生壳；Apple/微信登录、iOS `SaveFile` 与本地通知均已完成本轮 iPhone 验收。步骤 9 的签名、隐私、合规、App Store Connect 资料、`1.0.0 (4)` 基线回归已完成；当前源码已修正 App 内版本/备案展示，并上传最新 TestFlight 候选 `1.0.0 (12)`，等待 Apple 处理。AI 识图的真实样本识别率与成本也已验证。步骤 10 的本地基线、总回归证据和 App Store Connect 首次提交均已完成；该首次审核曾因 Guideline 2.1 App Completeness 要求补充资料，账号持有人已在审核沟通中附上实体 iPhone 录屏并回复，当前等待 Apple 后续处理。公安平台主体与 After Zero APP 已提交，等待审核；当前不能把工程误写成已公开上架的 iOS 产品。Flutter 继续封存。
