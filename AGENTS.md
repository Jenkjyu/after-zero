...
- iOS 步骤 2 已通过检查；步骤 3“Apple 登录与统一内部账户端到端闭环”云端已部署，仍差 iPhone 真机验收。用户已明确允许步骤 4、5、6、7 在此前停点未解除时先行实施：步骤 4 等微信审核、官方 iOS SDK 接入与真机验收；步骤 5 文件代码与本机构建已完成；步骤 6 通知代码与本地验证已完成；步骤 7 WebView/布局/键盘/手势基础适配与模拟器验证已完成。步骤 3～7 均等待集中真机验收；不能据此进入步骤 8。权威计划为 `docs/ios/implementation-plan.md`，准确恢复动作见 `docs/ios/handoff.md`。
