...
**协作节奏（每个阶段必须遵守）**：严格按下面的0～9阶段顺序推进；一个阶段的完整范围做完后，先跑与风险相称的自动测试和实际操作验证，再更新`README.md`、本文件（AGENTS.md）与本机的`PROGRESS.md`，然后**停下来等待用户确认**，才可以拆解并开始下一阶段。进行中的阶段只在`PROGRESS.md`记录临时进展，不能在这里或`README.md`提前写成“完成”，也不能趁当前阶段一次性规划或实现后续阶段。
...
**每一条当年具体怎么坏的、怎么修的、验证了多少用例，见`debt-model-history` skill**（`.agents/skills/debt-model-history/SKILL.md`）——通知调度窗口、导出含已结清债务、还款流水`paidAt`、部分还款分摊逻辑、amount一致性校验的0.015容差怎么算出来的、`d.day`死字段清理，都在里面。
...
```
**这个代码块之后不再逐步枚举新增函数**——`resetLocalData`（注销账户新增"仅重置本地数据"那轮）、`buildAiSummary`（AI额度弹窗"复制提示词"按钮那轮）都是这类例子，凡是vanilla需要暴露新的写操作给React调用，任何时候都可以照着现有几十个例子的模式加，不需要等"下一个迁移步骤"。这个代码块本身会滞后于实际代码，`grep "window.__azBridge = {" -A 40 www/index.html`看当前真实状态比信这里的快照可靠。**`generateHistoryShareCard`曾经在这个列表里（还债历程分享卡片那轮，2026-08-04），2026-08-05这个功能整个删除了，见"还债历程"一节。**
...
**SDK硬编码写死不能改的路径、CloudBase接线时按顺序会踩到的6个坑、`createTicket`/`signInWithCustomTicket`用法、release签名要求，全部见`wechat-login-setup` skill**（`.agents/skills/wechat-login-setup/SKILL.md`），不在这里重复。
...
**CDN脚本加载顺序、`signInAnonymously()`规避bug、匿名登录开关、`wxLogin`权限例外、`users`集合手建、release包WebView调试这6个坑，以及`createTicket`/`signInWithCustomTicket`API用法、"别启用内置微信开放平台登录"，全部见`wechat-login-setup` skill**（`.agents/skills/wechat-login-setup/SKILL.md`）。

**部署云函数、`cloudbaserc.json`要求、环境变量/权限控制的坑，见`cloudbase-deploy` skill**（`.agents/skills/cloudbase-deploy/SKILL.md`）——不在这里重复，那边有完整的部署命令、三个必查的坑、验证方法、当前全部云函数一览。
...
**⚠️`z-index`原来是25，2026-08-05一个后来被整体删除的功能（曾经把`Popover`用进`.subpage`内部）才暴露出这是个坑，细节见"还债历程"一节**：`.popover-panel`原来`z-index:25`，排在`.tabbar`(20)和`.scrim`(30)之间——这在`Popover`只被`InfoTip.tsx`/`ExportMenu.tsx`用在普通`.view`标签页内容里时够用（那层context本身z-index基线很低），但`.subpage`是35，比25高，面板虽然是`createPortal`挂到`document.body`、DOM顺序也在后面，**z-index比较不看DOM顺序，只看数值**——于是面板被扣在`.subpage`视觉下方，`visibility:visible`、DOM里数据也对，肉眼却完全看不见。**这个bug是Playwright真实截图比对(点了没反应)才发现的，代码审查看不出来**——面板的定位/内容逻辑一行都没错，纯粹是两个数字谁大谁小的问题。改成`z-index:36`（高于`.subpage`，低于`.login-gate`=40），覆盖"`Popover`被用在`.view`/`.sheet`/`.subpage`任意一层"这三种已知场景，以后新增的`Popover`调用点不用再操心这件事。**触发这个坑的功能本身已经删除，但这条修复是通用的正确性修复，不要因此回滚。**
...
急迫程度4档阈值、分组(`dueBucket`)跟筛选(`PayFilter`)两套"7天内/30天内"语义为什么不同（互斥分段 vs 累计口径）、左滑手势实现细节（`__justDragged`标记复位坑）、筛选条5档+日历自定义天数改版，全部见`pay-tab-design` skill（`.agents/skills/pay-tab-design/SKILL.md`）。
...
全项目最复杂的一块UI（公式生成器、批量设置还款日、`oneTimeStash`状态机）。`oneTimeStash`暂存机制、`planMode`切换、`#gFirstField`的DOM搬家技巧、批量设置日期确认框、29/30/31号限制、计息方式选择器+等额本金新增、`genPlan()`四舍五入/负数bug的完整排查过程，全部见`edit-sheet-design` skill（`.agents/skills/edit-sheet-design/SKILL.md`）。
...
> - **只有一张价卡，¥15，没有促销话术、也没有说明文案**：`react/src/sheets/PremiumScreen.tsx` 的 `#premiumPrice` 只剩一张静态 `<div className="price-card">`（不再是可点击切换选中态的 `<button>`，`premiumPlanSel`/`Plan` 类型/`useState` 一并删除——只有一个选项，没有"选"这个动作）。**价格改过两次**：¥24原价¥40划线+"限时优惠"角标（已删）→ 朴素¥49（"云备份和AI分析对我们来说有真实的服务器成本……"那句朴素说明是这一版加的，解释免费/付费边界怎么划）→ **2026-08-05用户直接要求改成¥15，同时把那句说明文案整段删掉**（连同解释这句话为什么存在的代码注释一起删）——不再需要在价卡旁边解释成本，卡片外框去掉了常驻的 `.selected` 高亮（只剩一张卡时"选中态"这个语义已经不存在）。**具体数字是产品决策，不是技术细节，改的时候别自己套别的框架**（比如按百分比"省X%"去算——真机验证时用户明确纠正过这一点）。
> - **免费/付费边界**：图表查看、提前还款模拟器、**多策略对比规划的免费部分（对比结果）** → **免费**（零成本、口碑引擎，删了门禁）；高级统计报表**导出 PDF/Excel**、云备份、AI → **付费**（导出在 `reportExportXlsxBtn`/`reportExportPdfBtn` 的 click handler 里判 `hasPremium()`，查看图表无门禁）。判断标准始终是：有真实成本（服务器/算力）才收费，纯客户端零成本的不设障碍——见下面"多策略对比规划"一节，这条边界2026-08-04分析付费策略那轮被重新审视过一次、结论是维持不变（不能为了转化率就打破这条已经对用户诚实过一次的原则）。（原来这里还提过"还债历程的基础时间线免费/生成分享卡片付费"，这个功能2026-08-05整个删除了，见"还债历程"一节。）
...
**⚠️2026-08-05修的一个真机bug：只要有一笔"先息后本"债务处在利息期（还没进入还本阶段），整个多策略对比就会全军覆没（三种策略同时返回null，UI显示"月供覆盖不了利息，本金永远还不完"），删掉那笔债务功能立刻恢复正常——用户报的现象，且这不是这一笔债务本身有问题，是任何一笔先息后本债务处在利息期都会触发这条误判。**

根因在`simulateRepaymentOrder`（以及同一套简化模型下的`amortForward`，被"提前还款收益模拟器"复用，同源问题理论上也会在那边触发，只是还没被真机撞到）的"月供是否覆盖得了利息"预检查——原来写的是死板的`min < balance*i - 0.005`。这条检查的两个输入`balance`/`rate`都是`recompute()`里`r2()`舍入过的派生值，尤其`d.rate`（`impliedAPR`反推的隐含年化）本身就带着量级在±0.01个百分点的舍入误差（实测：先息后本债务生成时用的原始利率是5.81%，IRR反推经`r2()`舍入后变成5.82%），拿这个舍入过的利率反推"这期利息应该是多少"再去跟真实的下一期还款金额比，会出现一个纯粹是舍入噪声、跟这笔债务是不是真的还不完毫无关系的差值——而**先息后本类债务在利息期，月供设计上就是恰好等于利息**（本金分文不动，直到进入还本阶段），这正是这条检查最容易被舍入噪声顶过0.005这道极窄门槛的场景：一笔完全正常、按计划走的先息后本债务，被误判成"利息都覆盖不了、永远还不完"。

实测过（不是理论推导）：拿genPlan生成的先息后本债务，balance从500扫到200万、rate从1%扫到36%、ni/np/已还期数各种组合，`(balance*i - monthly)/balance`这个比值稳定在1e-5量级，不随balance变化——是真实存在、有规律的舍入噪声，不是偶发的浮点误差。

**修法**：`www/js/calc.js`新增`interestCoverTolerance(balance)`，容差不再是写死的0.005，改成`Math.max(0.02, balance*0.00005)`（5倍安全余量覆盖上面实测的噪声量级），`amortForward`的循环内检查和`simulateRepaymentOrder`的逐笔预检查都改用这个公用容差；`amortForward`里`principal = Math.min(M-interest, bal)`同时补了`Math.max(0, ...)`兜底——放宽容差后`M`允许略低于`interest`（在容差内),那一期`principal`会算出一个微小负数，钳掉避免余额被舍入噪声反向推高。**这不只是浮点数误差修补**：容差调宽之后，先息后本债务在"排在队列后面、还没被雪球/雪崩集中火力"时，月供只够付利息、principal贡献钳成0——这恰好如实模拟了先息后本债务在利息期的真实行为（本金原地不动，直到被这套策略引擎的额外资金砸中或进入还本阶段才开始下降），不是把检查关掉，真正月供只有利息一半这种数据依然会被拦住（`interestCoverTolerance(1000)`量级在0.02~0.05这种小数目，不是能盖住任何情况的大数）。`test/calc.test.js`补了3条回归测试，用genPlan复现了这个真实场景（不是手写raw数字），外加验证"容差没有被放宽到失去意义"。

**⚠️2026-08-05又改了三处UI，都是用户真机截图指出的观感问题，不是功能bug**：
1. **结果卡片`.strat-result-row.best`原来有一圈`border-color:var(--accent)`+`box-shadow`描边**——这套"强调色描边"在这个App里到处都是"可点选中态"的视觉语言（价格卡选中态、分段控件选中态），但这三行是纯展示、不可点，用户反馈"给人一种另外两个也能点击选中的错觉"。删掉这圈描边，"最省"这件事完全交给旁边`.best-badge`文字胶囊说清楚，不需要再借控件视觉语言重复表达一遍。
2. **加粗大字的¥总利息数字原来没有任何文字说明它是什么**，用户反馈"莫名其妙"——补了一行`.kk`小字标签"总利息"在数字上方，跟这个App其它KPI卡片"大数字+小字说明"的既有模式统一。
3. **对比曲线图(`StrategyChart.tsx`)原来是判断"跟`Journey.tsx`同一个道理不画坐标轴"，但比`Journey.tsx`更彻底——连里程碑文字标注都没留**，用户反馈"图坐标都没有，不知道想表达什么"。这条判断确实过头了：`Journey.tsx`至少有三个里程碑文字标在图上撑住方向感，这张图什么定向线索都没留。补了两行最起码的方向提示（不是完整刻度轴）：左上角"剩余待还 ¥X"（3条线起点相同，标一次够了，同时点明Y轴含义）、右下角"时间→"（点明X轴是往右流逝的时间）。**为了让这两行能相对"绘图区自己的盒子"定位（不用去猜下面图例多高再拿外层容器的bottom反推），新增了一层`.strat-chart-plot`只包`<svg>`和这两行提示，图例`.strat-legend`留在这层外面。**

### ⚠️同一个bug的第二次、更深的修复（2026-08-05，用户上传真实备份文件才查出来）

上面那次`interestCoverTolerance`的修法**没有根治这个bug**，只是缩小了触发范围——它假定"月供覆盖不了利息"这个偏差只来自`impliedAPR`的`r2()`舍入噪声（量级~1e-5），但用户后续真实反馈"每月额外投入5000，总负债33935，为什么还要十几个月"，追查方式是**直接要来用户的本地备份JSON文件、用Node跑这个项目自己的`calc.js`函数验证**（不是靠肉眼审代码或猜）：

```
node -e 'var calc=require("./www/js/calc.js"); var data=JSON.parse(require("fs").readFileSync("备份文件路径")); ...'
```

这一验证就发现两件事都是真的但都不是对方想的那个原因：①用户记忆里的"33935"跟`summarizeDebts()`实际算出来的"在还总负债"（¥61,587.76）对不上，用户记错了/口算的是别的数字，这部分不是bug；②但**多策略对比规划本身确实在用这份真实数据时对全部三种策略都返回`null`**——根因还是那条"月供覆盖不了利息"检查，只是这次的偏差不是舍入噪声：一笔"交行惠民贷"（¥25000，先息后本，名义年化8.64%）的还款计划里，**首期利息被记成300、后续每期都是180**——因为放款日是23号，首期覆盖的天数跟后续按自然月计算的周期长度不同，是完全真实合理的银行放款惯例，不是数据错误。`impliedAPR`反推整份计划（含这条偏高的首期）算出的隐含年化是8.95%，比真实名义利率8.64%高出0.31个百分点——**这个偏差比第一次修的舍入噪声（~1e-5相对量级）大了几个数量级**，`interestCoverTolerance`那点容差完全盖不住，一笔完全正常、按计划还着的债务被判定成"利息都覆盖不了"，连累整个多策略对比返回`null`。

**真正的修法不是继续加大容差（容差多大都会有更极端的真实数据把它顶穿），是认识到这条预检查本身多余**：它逐笔独立判断"这笔债务自己的月供能不能覆盖自己的利息"，却没考虑这笔债务一旦排到队首会额外拿到`rollover`（其它已还清债务滚过来的月供+每月额外投入）——只要它排队等待期间没有恶性到让余额在`MAX_MONTHS`(600个月)内都收敛不了，轮到它成为队首后`payment=min+rollover`几乎总能覆盖利息、正常开始还本金。`simulateRepaymentOrder`里整段逐笔预检查直接删掉，真正"无论如何都还不完"的极端情况依然会被已有的`month>=MAX_MONTHS`兜底返回`null`，不会死循环；这笔债务排队等待期间余额会因为月供不够利息缓慢增长（负摊还），这如实反映了"钱没投给它、它自己利息又刚好打平或略亏"的真实情况，不是要掩盖的错误。`amortForward`（被单笔债务的"提前还款收益模拟器"复用，没有`rollover`这个救援机制）**没有改**，那边的检查继续保留，是两个函数的场景本质不同，不是漏改。

修完之后拿用户的真实备份重新跑：雪球法/雪崩法在额外投入¥5000/月时分别是15个月和14个月（不投入是19/18个月）——不再是`null`了，但**这个数字本身还是错的，是同一次追查里发现的第三个bug**，见下面这节。`test/calc.test.js`补了一条回归测试，直接构造"首期利息高于后续期"这种真实场景（不是舍入噪声那种极小偏差）验证不再误判。

### 第三个bug（同一次追查、当场发现）：资金池只喂给队首那一笔，喂不完的钱直接作废

上一节的bug修完、`null`变成了数字之后，用户追问一个非常朴素的问题："总负债六万多，每月多投5000，怎么还要十几个月？14×5000都七万了"——这个疑问经得起推敲，逐月拆解模拟结果（`res.monthly`）就直接暴露了问题：**第1个月，理论预算(全部债务月供合计¥4794.69+额外投入¥5000=¥9794.69)里，实际只花出去了¥4795，几乎整整¥5000的额外投入完全没用上。**

**根因**：`simulateRepaymentOrder`原来的逻辑是"资金池(已结清债务的月供+额外投入)整个给队列里第一笔还没还清的债务"——但如果排在队首的恰好是笔很小的债务（用户这份数据里雪球法排序第一名是余额只有¥100的"test1"），这笔债务只需要¥100就能结清，资金池里剩下的¥4900**没有代码把它接着喂给排第二的债务**，直接凭空消失，下个月也不会补回来。这不是舍入误差，是实打实的逻辑漏洞——**这个bug在"每月额外投入"这个功能的最核心场景下（多投钱、想让排前面的小额债务被迅速清掉）几乎必然触发**，因为雪球法本来就是刻意把小额债务排在前面的。

**修法**：整个月的处理拆成两轮，模拟真实理财顾问会怎么分配这个月的钱：
1. **第一轮**：每笔还活着的债务先按自己的月供结算利息+本金，但月供封顶在"这笔真实还欠多少"（通常只有最后一期会触发，比如本来月供200但只剩7.6没还完）——月供比实际需要的多出来的部分、以及已经结清的债务本该出的月供，都收进这个月的资金池。
2. **第二轮**：资金池按队列优先级顺序追加本金（利息不重复算，已经按月初余额在第一轮结算过了）——一笔追到位、还有剩就接着往下一笔追加，直到池子耗尽或者队列走完。

**为什么要分两轮，不能在第一轮里"顺手"把池子也发下去（第一版就是这么写的，被自己的回归测试打回来过）**：如果队列里排在后面的某笔债务在**第一轮执行到一半**才发现自己靠自己的月供就能提前结清、省出一截零头，这笔零头理论上应该回流给已经在第一轮处理过的、排在它前面的高优先级债务——但那些债务在"单轮往后流"的写法里已经处理完了，零头没有路径回得去。这条回归被现成的一条测试直接测出来（"雪崩法总利息不该比雪球法更多"这个数学性质，在特定排序组合下被打破了），逼着改成"先各自结算、再统一按优先级追加"这个两轮版本——第二轮开始时资金池已经汇总了全月所有来源的钱，按优先级从头追加，不存在"谁先处理过就摸不到后面发现的钱"这个问题。

**修完拿用户真实数据重新验证**：每月额外投入¥5000，雪球法/雪崩法都变成**7个月**（总利息¥2000出头），不投入是14个月——这才是跟"六万多总负债、每月多出五千"这个数量级相符的结果。`test/calc.test.js`补了两条测试：一条直接复现"队首是笔远小于额外投入的债务"这个场景，断言第1个月的余额必须大幅下降（不能停在原地）；另一条是把"雪崩法总利息不该比雪球法更多"这条本来就有的测试的注释补全，记录它曾经被两轮版本之前的写法打破过、又是怎么修回来的。

**教训（这次同一天里已经是第二次踩到同一类教训）**：这次能一路查到底，靠的是**用户愿意直接把本地备份文件发过来、拿这个项目自己的`calc.js`在Node里跑一遍验证**，而不是隔着截图猜测或者空口解释"这是数学上合理的结果"——如果当时止步于"债务多、利息高，久一点很正常"这类听起来有道理但没有验证过的解释，这个真正的bug会一直藏着。**遇到用户对着一个具体数字反问"这个不对吧"，且这个数字是可以用现成代码复现验证的，直接跑一遍比继续讲道理可靠得多。**

## 还债历程——2026-08-04新增，2026-08-05整个删除，别再加回来

统计tab末尾曾经有一个"还债历程"入口（时间线列表→后来重做成蜿蜒发光轨迹，中间还经历了分享卡片功能的增删），2026-08-05用户明确反馈"这个功能直接去掉，太傻逼了"，整个删除，不是缩减/重做。

删除范围：`react/src/report/HistoryTeaser.tsx`（统计tab入口卡片）、`react/src/sheets/HistoryScreen.tsx`（详情屏幕）、`react/src/sheets/JourneyPath.tsx`+`journeyGeom.ts`（蜿蜒轨迹渲染+几何计算）、对应的4个测试文件；`www/js/calc.js`的`buildHistoryEvents`/`buildHistoryJourney`两个函数+`HISTORY_MILESTONE_STEPS`常量（连同`test/calc.test.js`里11条测试）；`react/src/shared/state.ts`的`openHistoryScreen`/`closeHistoryScreen`/`useHistoryScreenOpen`；`www/index.html`里`__handleBackButton`链的`__azHistoryScreenBack`判断+全部`.history-*`/`.journey-*`/`.jr-*` CSS；`react/src/types.ts`的`HistoryEvent`/`JourneyEvent`/`DebtJourney`类型+`calcGlobals.d.ts`对应声明；`framer-motion`这个npm依赖（专门为这个功能引入的动画库，没有别的地方用，一并卸载，`sheets.js`因此从266KB降回159KB）。

`Popover`组件的`z-index`从25改成36那次修复**没有跟着回滚**——虽然触发它的场景（这个功能把`Popover`用进`.subpage`）已经不存在了，但这条修复本身是通用的正确性修复，不是这个功能专属的，别因为功能删了就把它也改回去，见"`Popover`定位"一节。

**这条记录留着的目的是防止以后重新加一个类似的"历程时间线"功能时又踩一遍已经踩过的坑（分享卡片的Premium边界怎么划、进度百分比分母该用什么、declutter压缩算法怎么写、手绘图形不如借用已有视觉语言）**——完整的设计过程和三次踩坑经过在git历史里（`git log -p --all -- react/src/sheets/HistoryScreen.tsx`），不在这里逐字复述。
...
**2026-08-05应用户要求删了三处提示/免责文案**：①甜甜圈下方"按住饼图可以转动"这行提示
（`.pie-hint`）——`pieRotate.ts`里原来有一处代码注释拿这行文案当"为什么边缘触摸精度不用
太计较"的理由，文案删掉后这条注释也改写了，不再依赖一句已经不存在的UI文案当设计理由。
②走势图（`Journey.tsx`）下方"按现有还款计划推算，不含提前还款；本App不保存历史余额，
这条线不是实际走过的轨迹"这句免责footnote整段删除——**这条事实本身没变**（timeline依然是
预测不是真实历史），只是页面上不再有文字提示这一点，`Journey.test.tsx`里专门测这句话的
那条测试也一并删掉。③页面最下面原来单独常驻的一行`.rpt-foot`（"利率由每笔债务的还款
计划反推（IRR）；剩余待付利息按现有计划算到还清为止……"）合并进了上面可折叠的"计算口径
说明"（`.rpt-note`），变成第七条口径——不再单独常驻占地方，用户反馈这行小字常驻在页面
最底部意义不大，跟其它口径说明本来就是同一类"较真的人才会展开"的细则。
...
"我的"页"云备份"入口打开`#backupScreen`（`react/src/sheets/BackupScreen.tsx`）：手动、每次创建一条独立备份记录（不是自动同步/覆盖），5个云函数+配额数字+集合寻址方式+客户端恢复逻辑+真机验证边界，全部见`cloud-backup-design` skill（`.agents/skills/cloud-backup-design/SKILL.md`）。
...
"在还债务"页AI banner是入口（`hasPremium()`门禁），聊天式界面（`react/src/sheets/AiScreen.tsx`）。欢迎态芯片、云函数模型选型（`hy3`混元，DeepSeek被套餐锁住）、每日用量软上限、历史对话可继续追问的状态机、z-index坑，全部见`ai-advisor-design` skill（`.agents/skills/ai-advisor-design/SKILL.md`）。
...
**要测微信登录必须编译release包**（debug签名过不了微信的签名校验）——release签名文件位置、构建命令、丢失后果见`release-keystore` skill（`.agents/skills/release-keystore/SKILL.md`）。
