...
- iOS 步骤 2 已通过检查；步骤 3“Apple 登录与统一内部账户端到端闭环”云端已部署，仍差 iPhone 真机验收。用户已明确允许步骤 4“iOS 微信登录、身份绑定与既有云账号合并”及步骤 5“iOS 文件保存、分享与导入导出闭环”在此前停点未解除时先行实施：步骤 4 等微信审核、官方 iOS SDK 接入与真机验收；步骤 5 代码与本机构建已完成，等待 iPhone 真机文件验收。权威计划为 `docs/ios/implementation-plan.md`，准确恢复动作见 `docs/ios/handoff.md`。
...
- 根 `ios/` 已是可编译、可在模拟器冷启动的 Capacitor 8.4.1 Swift Package Manager 原生壳；Apple 登录云端已部署但尚未真机验收，iOS `SaveFile` 文件保存/分享代码已实现但尚未真机验收；微信、完整通知、购买和上架仍未实现，不能把原生壳误写成可发布 iOS 产品。Flutter 继续封存。
