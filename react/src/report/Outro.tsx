// 页尾：结语（"如果只做一件事"）+ 导出入口 + 计算口径说明。
//
// "如果只做一件事"的内容跟"最该先动手的地方"取的是**同一条结论**（severity 最高的
// 可行动那条），这是刻意的重复：开头给判断、中间给证据、结尾再落回同一个动作，
// 是报告体的收尾方式，不是复制粘贴。
//
// ⚠️"计算口径说明"必须保留。它解释的七条（在还总负债只算本金 / 已还本金含已结清 /
// 经常性月供不含一次性还清 / 归零进度只按本金 / 预计还清是预测 / 提前结清怎么记账 /
// 利率是IRR反推)都是这一页数字的口径前提，删了会让好几个数字变得无法核对。改版把它
// 从顶部挪到页尾，是因为它是"较真的人才会展开"的补充细则，不该占第一屏。
// ⚠️2026-08-05：原来单独常驻显示的一行`.rpt-foot`（利率反推IRR + 剩余利息口径说明）
// 已经合并进这个可折叠的口径说明里——用户反馈那行小字常驻在页面最底部意义不大，
// 跟其它口径说明本来就是同一类"较真的人才会展开"的细则，不需要单独常驻占地方。
    ...
    <>
      ...
      {noteOpen && (
        <div className="rpt-note">
          ...
          「提前结清」会问你实际付了多少钱，并把剩余期次合并成一条结清记录：剩余本金计入已还本金，实付超出剩余本金的部分计入另付利息（协商减免则记为负数）。未来那些期原本的利息不会被算成你付过——提前结清本来就免掉了它们。<br />
