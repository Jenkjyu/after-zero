...
declare global {
  interface Window {
    ...
    // 返回null只有一种情况：超过600个月(50年)还没还完。debts只需要{id,balance,rate,monthly}
    // 这4个字段，不要求完整Debt对象。2026-08-05删掉了"逐笔单独判断月供覆盖不了利息就直接
    // 拒绝"那条预检查——见calc.js注释，真实用户数据(先息后本债务第一期不规则)会误伤。
