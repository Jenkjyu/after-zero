  ...
  return (
    <div className={"subpage" + (isOpen ? " open" : "")} id="termsScreen">
      ...
      <div className="subpage-body terms-body">
        ...
        <h3>一、服务内容和会员权益</h3>
        <ol>
          <li>Premium 是本产品目前唯一的付费会员等级，开通后可使用：还款日与通知提醒、统计债务报告、云备份、档案库、备份文件导入导出、多策略对比规划、AI 识图录入等功能。具体权益范围以您使用时应用内实际展示为准，我们可能根据产品发展调整全部或部分权益，调整会在应用内公告。</li>
          <li>人民币 28 元买断 Premium 包含 <strong>25 次终身 AI 识图录入额度</strong>；iOS 首次登录后的 7 天 Premium 体验另含 <strong>3 次 AI 识图录入额度</strong>。体验额度与买断额度独立计算，体验期结束后未使用的体验额度不会转入买断额度。同一笔债务可选择多张还款计划截图组成一组；一次识别成功生成一份可编辑草稿时消耗 1 次额度，多张截图仍只计算 1 次，识别失败或未生成草稿不消耗额度。草稿需由您检查、补齐并确认后才会写入本地账本。</li>
          <li>提前还款模拟目前对所有用户免费开放，不属于 Premium 专属权益；是否收费我们可能根据成本情况调整并提前公告。</li>
        </ol>
        ...
        <ol>
          ...
          <li>未经我们明示授权，任何转售、批量分发本产品会员资格的行为均属违规，我们有权取消相应资格并追究相关责任。</li>
