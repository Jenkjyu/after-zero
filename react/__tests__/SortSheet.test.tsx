...
describe("SortSheet", () => {
  it("关闭时不带open类(常驻挂载、只切class，这样上滑过渡才播得出来)", () => {
    ...
    expect(document.querySelector(".option-sheet")).not.toHaveClass("open");
  ...
  it("打开后列出全部选项，当前项带active和aria-current", () => {
    ...
    expect(document.querySelector(".option-sheet")).toHaveClass("open");
  ...
  it("内容在.sheet-scroll内层滚动容器里，grip留在外层", () => {
    ...
    const sheet = document.querySelector(".option-sheet")!;
    const scroll = sheet.querySelector(".sheet-scroll")!;
    expect(scroll).toBeTruthy();
    expect(scroll.querySelector(".option-list")).toBeTruthy();
