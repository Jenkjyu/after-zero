...
describe("DebtCard", () => {
  ...
  it("点击'销这期'按钮调用__azBridge.payInstallment(i)，并收起滑出状态", () => {
    ...
    const d = makeDebt();
    const ctx = makeCtx();
    const { container } = render(<DebtCard d={d} i={5} jiggleMode={false} ctx={ctx} />);
    ...
    expect(window.__azBridge.payInstallment).toHaveBeenCalledWith(5);
  ...
  it("利率>=18%时使用crit严重度色晕类，>=10%用warn，其余用good", () => {
    window.__azBridge = makeMockBridge();
    const critEl = render(<DebtCard d={makeDebt({ rate: 20 })} i={0} jiggleMode={false} ctx={makeCtx()} />).container.querySelector(".debt")!;
    expect(critEl.className).toContain("crit");
    const warnEl = render(<DebtCard d={makeDebt({ rate: 12 })} i={0} jiggleMode={false} ctx={makeCtx()} />).container.querySelector(".debt")!;
    expect(warnEl.className).toContain("warn");
    const goodEl = render(<DebtCard d={makeDebt({ rate: 5 })} i={0} jiggleMode={false} ctx={makeCtx()} />).container.querySelector(".debt")!;
