...
describe("PayList", () => {
  ...
  it("按dueBucket分组，插入section-label且带正确计数", () => {
    window.__azBridge = makeMockBridge();
    const items: PayItem[] = [
      { d: makeDebt({ name: "逾期A" }), next: new Date(), diff: -2 },   // overdue
      { d: makeDebt({ name: "7天内B" }), next: new Date(), diff: 3 },   // week
      { d: makeDebt({ name: "7天内C" }), next: new Date(), diff: 5 },   // week
      { d: makeDebt({ name: "30天内D" }), next: new Date(), diff: 20 }, // month
    ];
