...
describe("Stats", () => {
  ...
  it("7天内/30天内待还是累计口径(30天内包含7天内)，逾期不计入", () => {
    const items: PayItem[] = [
      { d: makeDebt({ monthly: 100 }), next: new Date(), diff: -1 }, // 逾期，不计
      { d: makeDebt({ monthly: 200 }), next: new Date(), diff: 3 },  // 7天内
      { d: makeDebt({ monthly: 300 }), next: new Date(), diff: 20 }, // 30天内但不在7天内
      { d: makeDebt({ monthly: 400 }), next: new Date(), diff: 45 }, // 超过30天，不计
    ];
