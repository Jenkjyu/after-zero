...
本文只记录步骤 9 的签名、隐私、合规和 TestFlight 候选状态，以及步骤 10 的发布收尾；步骤 9、10 均已完成。Apple 于 2026-09-01 对 `1.0.0 (12)` 提出三项要求后，修复已落在当前源码和云函数，`1.0.0 (13)` 已通过审核并正式上架。真实 iPad 验证按用户决定不再阻塞；步骤 4/8 的剩余小项按用户决定延后到上架后。
...
- App Store Connect 隐私标签、年龄分级、分类、截图、支持 URL、隐私政策 URL、审核备注、测试账号/演示路径和出口合规信息已完成填写与确认。Guideline 3.1.1 的自定义兑换码已移除；Guideline 4.2.3(i) 的 iOS 微信登录和绑定入口已隐藏；Guideline 2.1(a) 的 iPad 拍照入口已补齐权限说明。修复后的 `1.0.0 (13)` 已通过审核并正式上架。
...
- 步骤 10 的本地构建与总回归已完成；用户确认沿用既有 iPhone 验收证据，不再因本机 CoreDevice 服务不可用追加真机阻塞。`1.0.0 (13)` 已通过审核并正式上架。
