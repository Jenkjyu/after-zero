...
状态：步骤 0–2 已批准；步骤 3、5、6、7 的本轮 iPhone 验收已完成；步骤 4 的 Apple→微信绑定已通过，其余账户组合与合并后备份可见性按用户决定延后到上架后。步骤 8 的 CloudBase 与 App Store Connect 通知配置已完成，剩余 StoreKit 真机生命周期验收按用户决定延后到上架后，不阻塞当前发布。步骤 9 已完成；AI 识图真实样本识别率与成本也已验证。步骤 10 的首次审核于 2026-09-01 收到三项修复要求：移除自定义兑换码、iOS 不再要求安装微信登录、修复 iPad 拍照入口崩溃；修复已在源码和云端完成，iOS 当前仅呈现 Apple 登录，修复后的 `1.0.0 (13)` 已上传 App Store Connect 并由账号持有人提交审核，当前等待 Apple 处理。真实 iPad 验证按用户决定不再阻塞。Android 回归降为内部测试事项，不阻塞 iOS 发布主线。
...
| 4 | iOS 微信登录、身份绑定与既有云账号合并 | 当前 iOS 发布版仅呈现 Apple 登录；微信保留 Android 内测路径，iOS 未来如恢复须采用合规的应用内网页授权方案 |
...
| 10 | iOS 总回归、Android 内测冒烟与 App Store 首次提交 | 首次提交已完成；2026-09-01 收到 3.1.1/4.2.3(i)/2.1(a) 修复要求，`1.0.0 (13)` 已上传并重新提交审核，当前等待 Apple 处理 |
