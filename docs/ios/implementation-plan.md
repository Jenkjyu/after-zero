...
状态：步骤 0–2 已批准；步骤 3 等待 iPhone 真机验收；步骤 4 经用户明确允许提前进行，等待微信审核及真机验收
...
| 2 | 本地优先模式与云功能登录门 | 已批准 |
| 3 | Apple 登录与统一内部账户端到端闭环 | 阻塞：云端已部署，等待 iPhone 真机验收 |
| 4 | iOS 微信登录、身份绑定与既有云账号合并 | 阻塞：用户允许提前实施；等待微信审核、iOS SDK 接入与真机验收 |
| 5 | iOS 文件保存、分享与导入导出闭环 | 阻塞：经用户明确例外授权提前实施；代码和本机构建完成，等待 iPhone 真机验收 |
| 6 | Android/iOS 本地通知双平台闭环 | 阻塞：经用户明确例外授权提前实现；代码和本地验证完成，等待集中真机验收 |
| 7 | iOS WebView、布局、键盘与手势全量适配 | 阻塞：经用户明确例外授权提前实现；基础适配与模拟器验证完成，等待 iPhone 真机验收 |
