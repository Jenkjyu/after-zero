...
状态：步骤 0–2 已批准；步骤 3、5、6、7 的本轮 iPhone 验收已完成；步骤 4 的 Apple→微信绑定已通过，其余账户组合与合并后备份可见性按用户决定延后到上架后。步骤 8 的 CloudBase 与 App Store Connect 通知配置已完成，剩余 StoreKit 真机生命周期验收按用户决定延后到上架后，不阻塞当前发布。步骤 9、10 均已完成；AI 识图真实样本识别率与成本也已验证。步骤 10 的审核修复已在源码和云端完成，iOS 当前仅呈现 Apple 登录，`1.0.0 (13)` 已通过审核并正式上架。真实 iPad 验证按用户决定不再阻塞。Android 回归降为内部测试事项，不阻塞 iOS 发布主线。
...
| 10 | iOS 总回归、Android 内测冒烟与 App Store 首次提交 | 已完成；2026-09-01 审核修复后的 `1.0.0 (13)` 已通过审核并正式上架 |
...
- `1.0.0 (13)` 已通过 App Review 并正式上架；后续维护事项按用户具体授权处理。
