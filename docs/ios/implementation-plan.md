...
状态：步骤 0–2 已批准；步骤 3～7 的本轮 iPhone 验收已完成，仍待对应 Android 回归；步骤 4 的 Apple→微信绑定已通过，其他账户组合仍待补齐
...
- 现有持久化键名、包名、账本算法、Android 发布身份和 Flutter 封存门禁继续受 `AGENTS.md` 保护。
...
| 2 | 本地优先模式与云功能登录门 | 已批准 |
| 3 | Apple 登录与统一内部账户端到端闭环 | 阻塞：iPhone Apple 全链路通过，待 Android 旧微信账号回归 |
| 4 | iOS 微信登录、身份绑定与既有云账号合并 | 阻塞：官方 iOS SDK、CloudBase 函数/私有集合和 iPhone Apple→微信绑定已通过；仍待其余账户组合与 Android 旧微信账号回归 |
| 5 | iOS 文件保存、分享与导入导出闭环 | 阻塞：iPhone Files 保存/打开/取消验收通过，待 Android SAF 回归；当前 UI 未暴露档案分享入口，用户确认本轮不阻塞 |
| 6 | Android/iOS 本地通知双平台闭环 | 阻塞：iPhone 真机验收通过，待 Android channel、450 条上限与重排回归 |
| 7 | iOS WebView、布局、键盘与手势全量适配 | 阻塞：iPhone 真机验收及新增债务滚动修复通过，待 Android 交互回归 |
| 8 | StoreKit Premium、恢复购买与服务端权益 | 进行中：本地实现与编译通过，待 CloudBase/App Store Connect 部署和真机 StoreKit 验收 |
| 9 | 签名、隐私、合规与 TestFlight 发布候选 | 未开始 |
| 10 | 双平台总回归与 App Store 首次提交 | 未开始 |
...
- 云函数单测/本地测试、部署日志、iPhone 真机、Android 回归和项目文档全部闭环。
...
- Android SAF 成功、取消和大文件回归通过；桌面浏览器 fallback 仍可用。
...
- Android 真机/模拟器仍可建立 channel、发送测试通知并保留最多 450 的现有策略。
...
- Android 浅深色、手势、硬件返回和 WebView 视觉无回归。
...
- AI、云备份、报告导出和策略入口权益一致；Android 回归通过。
...
- Android release 身份和构建不受影响；发布文档、skills、交接完整。
...
## 步骤 10：双平台总回归与 App Store 首次提交

### 完整范围

- 按真实空安装、旧 Android 升级、Apple 新用户、微信旧用户、绑定/合并、多设备本地数据独立、手动备份恢复等矩阵做总回归。
