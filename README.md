...
"我的"页有一个**单一 Premium** 订阅入口，只提供一次性买断这一种购买方式（没有按月/按年订阅），目前只是UI占位（App还没上架应用商店，暂时接不了真实支付），为未来的付费功能铺路。免费/付费的边界是按"这个功能有没有真实服务器/算力成本"划的：统计报告本身、提前还款收益模拟器完全免费；报告页**导出**PDF/Excel、云备份（手动创建备份记录，可随时恢复）、**AI债务顾问**（雪球/雪崩法分析生成优化报告 + 针对自己债务数据的多轮问答，服务端调用腾讯云开发内置大模型，每天限量20次，超限后可以一键复制包含全部债务信息的完整提示词去问其他AI助手）是Premium会员专属功能。
