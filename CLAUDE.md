...
**`summarizeAllTime(debts)`（第42个）+ `computeUpcomingPressure(debts, monthsAhead, today)`（第43个）是"统计tab口径修正+压力图"这轮（2026-07-29）新增的，两个都是为了修掉已确认的真实口径bug，不是新功能包装**（完整背景见下面"统计"一节的"统计tab口径修正"子节）：
- **`summarizeAllTime`跟`summarizeDebts`只差一点**：已还本金/已还利息算全量（**含已结清债务**），其余字段（在还总负债/经常性月供/在还与已结清笔数/归零进度）口径完全一致、返回形状完全相同（是drop-in替换）。**为什么不直接改`summarizeDebts`**：那个函数被"债务"tab的hero（`debts/Summary.tsx`）共用，配套的口径说明footnote明写着"两者都不含已结清的债务"，是它自洽的局部口径；但"统计"tab语义上要的是真正的累计，复用`summarizeDebts`会导致**销掉最后一期→债务结清→"已还金额"和"归零进度"当场倒退**（用户刚还完一笔钱，统计数字反而变小）。独立成新函数、不动被别处共用的既有函数，跟`computeMonthlyRepayment`当初不并入`computeReportData`是同一个先例。⚠️口径细节：提前结清（`settleFull`）只写`settled=true`、不标记plan为已还，所以那笔债务的剩余本金**既不在`total`也不在`paidPrincipal`**——它是"用一笔金额未知的钱结掉了"，两边都不计是诚实的处理，UI的口径说明里必须讲清楚，不能假装它被还了。
